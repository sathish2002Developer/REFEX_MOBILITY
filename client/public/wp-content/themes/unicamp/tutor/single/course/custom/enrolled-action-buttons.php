...
if ( $lesson_url ) {
	$button_class = 'tutor-btn ' . ( $retake_course ? 'tutor-btn-outline-primary' : 'tutor-btn-primary' ) . ' tutor-btn-block' . ( $retake_course ? ' tutor-course-retake-button' : '' );
	...
	if ( $retake_course ) {
		$button_text = __( 'Retake This Course', 'unicamp' );
	} elseif ( $completed_percent <= 0 ) {
		$button_text = __( 'Start Learning', 'unicamp' );
	} else {
		$button_text = __( 'Continue Learning', 'unicamp' );
	}
	...
	$start_content = sprintf( '<%1$s %2$s class="%3$s" data-course_id="%4$s">%5$s</%1$s>', $button_tag, $attributes, esc_attr( $button_class . ' ' . $button_identifier ), esc_attr( get_the_ID() ), $button_text );
}
...
if ( ! $is_completed_course ) {
	...
	<form method="post" class="tutor-mt-20">
		...
		<button type="submit" class="tutor-btn tutor-btn-outline-primary tutor-btn-block" name="complete_course_btn"
			...
			<?php esc_html_e( 'Complete Course', 'unicamp' ); ?>
		</button>
	</form>
	<?php
...
if ( '' !== $post_date ) :
	...
	<div class="tutor-fs-7 tutor-color-muted tutor-mt-20 tutor-d-flex">
		...
		<div class="tutor-enrolled-info-text">
			<?php esc_html_e( 'You enrolled in this course on', 'unicamp' ); ?>
