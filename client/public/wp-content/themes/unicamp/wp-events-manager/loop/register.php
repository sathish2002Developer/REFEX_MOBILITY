...
<div class="entry-register">
	...
	<ul class="entry-event-info">
		...
		$date_start_meta = get_post_meta( get_the_ID(), 'tp_event_date_start', true );
		$date_end_meta   = get_post_meta( get_the_ID(), 'tp_event_date_end', true );

		$date_start = $date_start_meta ? ( new DateTime( $date_start_meta, $timezone ) )->getTimestamp() : time();
		$date_end   = $date_end_meta ? ( new DateTime( $date_end_meta, $timezone ) )->getTimestamp() : time();
	...
	<?php if ( $can_register ) : ?>
		...
		<?php } else { ?>
			...
			<p class="event-register-message">
				<?php echo sprintf( esc_html__( 'You must %1$slogin%2$s before register event.', 'unicamp' ), '<a href="javascript:void(0);" class="open-popup-login link-transition-01">', '</a>' ); ?>
			</p>
		<?php } ?>
	<?php else: ?>
