...
if ( ! class_exists( 'Unicamp_WP_Widget_Course_Categories' ) ) {
	class Unicamp_WP_Widget_Course_Categories extends Unicamp_WP_Widget_Base {
		...
		public function __construct() {
			$this->widget_id          = 'unicamp-wp-widget-course-categories';
			$this->widget_cssclass    = 'unicamp-wp-widget-course-categories';
			$this->widget_name        = esc_html__( '[Unicamp] Course Categories', 'unicamp' );
			$this->widget_description = esc_html__( 'A list or dropdown of course categories.', 'unicamp' );
			$this->settings           = array(
				'title'              => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Course categories', 'unicamp' ),
				...
				'hide_empty'         => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__( 'Hide empty categories', 'unicamp' ),
				),
				'max_depth'          => array(
					'type'  => 'text',
					'std'   => '',
					'label' => esc_html__( 'Maximum depth', 'unicamp' ),
					'desc'  => esc_html__( 'For eg: Input 1 to show only top categories.', 'unicamp' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( is_tax( 'course-category' ) ) {
				...
				if ( $terms ) {
					$main_term           = apply_filters( 'unicamp_course_categories_widget_main_term', $terms[0], $terms );
			...
			if ( $dropdown ) {
				Unicamp_Tutor::instance()->course_dropdown_categories( wp_parse_args(
					$dropdown_args,
					array(
						'show_count'         => $count,
						'hierarchical'       => $hierarchical,
				...
				if ( jQuery().selectWoo ) {
					var tutor_course_category_select = function() {
						jQuery( '.dropdown-course-category' ).selectWoo( {
							placeholder: '" . esc_js( __( 'Select a category', 'unicamp' ) ) . "',
			...
			"
				...
				$list_args['walker']                     = new Unicamp_Course_Taxonomy_Term_List_Walker();
				$list_args['title_li']                   = '';
				$list_args['pad_counts']                 = 1;
				$list_args['show_option_none']           = esc_html__( 'No categories exist.', 'unicamp' );
				...
				wp_list_categories( apply_filters( 'unicamp_course_categories_widget_args', $list_args ) );
