...
if ( ! class_exists( 'Unicamp_WP_Widget_Course_Duration_Filter' ) ) {
	class Unicamp_WP_Widget_Course_Duration_Filter extends Unicamp_Course_Layered_Nav_Base {

		public function __construct() {
			$this->widget_id          = 'unicamp-wp-widget-course-duration-filter';
			$this->widget_cssclass    = 'unicamp-wp-widget-course-duration-filter unicamp-wp-widget-filter unicamp-wp-widget-course-filter';
			$this->widget_name        = esc_html__( '[Unicamp] Course Duration Filter', 'unicamp' );
			$this->widget_description = esc_html__( 'Shows duration in a widget which lets you narrow down the list of courses when viewing courses.', 'unicamp' );
			$this->settings           = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by duration', 'unicamp' ),
		...
		protected function layered_nav_list( $instance ) {
			...
			$duration_ranges = array(
				'short'     => esc_html__( 'Less than 2 hours', 'unicamp' ),
				'medium'    => esc_html__( '3 - 6 hours', 'unicamp' ),
				'long'      => esc_html__( '7 - 16 hours', 'unicamp' ),
				'extraLong' => esc_html__( '17+ Hours', 'unicamp' ),
			);

			$duration_ranges = apply_filters( 'unicamp_widget_course_duration_filter_ranges', $duration_ranges );
		...
		protected function get_filtered_course_count( $duration ) {
			...
			$meta_query = Unicamp_Course_Query::set_meta_query_duration( $meta_query, (array) $duration );
