...
class Widget_Accordion extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Accordion/Toggle', 'unicamp' );
	}
	...
	public function get_script_depends() {
		return [ 'unicamp-widget-accordion' ];
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'style', [
			...
			'prefix_class' => 'unicamp-accordion-style-',
		] );
		...
		$this->add_control( 'type', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'accordion' => esc_html__( 'Accordion', 'unicamp' ),
				'toggle'    => esc_html__( 'Toggle', 'unicamp' ),
			],
			'default' => 'accordion',
		] );

		$this->add_control( 'active_first_item', [
			'label'              => esc_html__( 'Active First Item', 'unicamp' ),
		...
		$repeater->add_control( 'title', [
			...
			'default'     => esc_html__( 'Accordion Title', 'unicamp' ),
			'label_block' => true,
			'dynamic'     => [
				'active' => true,
			],
		...
		$repeater->add_control( 'content', [
			...
			'default' => esc_html__( 'Accordion Content', 'unicamp' ),
		] );
		...
		$this->add_control( 'items', [
			...
			'default'     => [
				[
					'title'   => esc_html__( 'Accordion #1', 'unicamp' ),
				...
				[
					'title'   => esc_html__( 'Accordion #2', 'unicamp' ),
					'content' => esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'unicamp' ),
				],
				[
					'title'   => esc_html__( 'Accordion #3', 'unicamp' ),
					'content' => esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'unicamp' ),
				],
			],
			'title_field' => '{{{ title }}}',
		...
		$this->add_control( 'active_icon', [
			'label'       => esc_html__( 'Active Icon', 'unicamp' ),
			'type'        => Controls_Manager::ICONS,
			'default'     => [
		...
		$this->add_control( 'title_html_tag', [
			'label'     => esc_html__( 'Title HTML Tag', 'unicamp' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'h1'  => 'H1',
				'h2'  => 'H2',
	...
	private function add_styling_section() {
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'name'      => 'box_shadow',
			'selector'  => '{{WRAPPER}} .accordion-section',
		] );
		...
		$this->add_control( 'items_spacing', [
			...
			'selectors'  => [
				'{{WRAPPER}} .unicamp-accordion .accordion-section + .accordion-section' => 'margin-top: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function add_icon_style_section() {
		...
		$this->add_responsive_control( 'icon_size', [
			...
			'selectors' => [
				'{{WRAPPER}} .unicamp-accordion .accordion-icons' => 'font-size: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'icon_space', [
			...
			'selectors' => [
				'body:not(.rtl) {{WRAPPER}} .unicamp-accordion.unicamp-accordion-icon-right .accordion-icons' => 'margin: 0 0 0 {{SIZE}}{{UNIT}};',
				'body:not(.rtl) {{WRAPPER}} .unicamp-accordion.unicamp-accordion-icon-left .accordion-icons'  => 'margin: 0 {{SIZE}}{{UNIT}} 0 0;',
				'body.rtl {{WRAPPER}} .unicamp-accordion.unicamp-accordion-icon-right .accordion-icons'       => 'margin: 0 {{SIZE}}{{UNIT}} 0 0;',
				'body.rtl {{WRAPPER}} .unicamp-accordion.unicamp-accordion-icon-left .accordion-icons'        => 'margin: 0 0 0 {{SIZE}}{{UNIT}};',
			],
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'unicamp-accordion' );
		...
		if ( $has_icon ) {
			$this->add_render_attribute( 'wrapper', 'class', 'unicamp-accordion-icon-' . $settings['icon_align'] );
		}
		?>
	...
	protected function content_template() {
		...
		view.addRenderAttribute( 'wrapper', 'class', 'unicamp-accordion' );
		...
		if ( iconHTML.rendered ) {
			view.addRenderAttribute( 'wrapper', 'class', 'unicamp-accordion-icon-' + settings.icon_align);
		}
