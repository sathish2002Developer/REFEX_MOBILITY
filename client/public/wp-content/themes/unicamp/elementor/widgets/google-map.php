...
class Widget_Google_Map extends Base {
	...
	public function get_title() {
		return esc_html__( 'Advanced Google Map', 'unicamp' );
	}
	...
	public function get_script_depends() {
		return [ 'unicamp-gmap3', 'unicamp-maps', 'unicamp-widget-google-map' ];
	}
	...
	private function add_map_options_section() {
		...
		$this->add_control( 'style', [
			'label'   => esc_html__( 'Map Style', 'unicamp' ),
		...
		$this->add_control( 'type', [
			'label'   => esc_html__( 'Map Type', 'unicamp' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__( 'Roadmap', 'unicamp' ),
				'satellite' => esc_html__( 'Satellite', 'unicamp' ),
				'hybrid'    => esc_html__( 'Hybrid', 'unicamp' ),
				'terrain'   => esc_html__( 'Terrain', 'unicamp' ),
			],
			'default' => 'roadmap',
		] );

		$this->add_control( 'zoom_level', [
			'label'       => esc_html__( 'Zoom Level', 'unicamp' ),
			'placeholder' => '12',
		...
		$this->add_control( 'scrollwheel', [
			'label'   => esc_html__( 'Mouse Scroll Wheel Zoom', 'unicamp' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'draggable', [
			'label'   => esc_html__( 'Draggable', 'unicamp' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'map_controls_heading', [
			'label'     => esc_html__( 'Controls', 'unicamp' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );

		$this->add_control( 'zoom_control', [
			'label' => esc_html__( 'Zoom Control', 'unicamp' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'scale_control', [
			'label' => esc_html__( 'Scale Control', 'unicamp' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'fullscreen_control', [
			'label' => esc_html__( 'Fullscreen Control', 'unicamp' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'street_view_control', [
			'label' => esc_html__( 'Street View Control', 'unicamp' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function add_map_markers_section() {
		$this->start_controls_section( 'markers_section', [
			'label' => esc_html__( 'Markers', 'unicamp' ),
		] );

		$this->add_control( 'marker_style', [
			'label'       => esc_html__( 'Marker Style', 'unicamp' ),
			'description' => esc_html__( 'Select default style for all markers', 'unicamp' ),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__( 'Icon', 'unicamp' ),
				'signal' => esc_html__( 'Signal', 'unicamp' ),
			],
			'default'     => 'icon',
		] );

		$this->add_control( 'marker_icon', [
			'label'     => esc_html__( 'Marker Icon', 'unicamp' ),
		...
		$repeater->start_controls_tab( 'marker_content_tab', [
			'label' => esc_html__( 'Marker', 'unicamp' ),
		] );

		$repeater->add_control( 'address', [
			'label'       => esc_html__( 'Address or Coordinate', 'unicamp' ),
		...
		$repeater->start_controls_tab( 'marker_popup_tab', [
			'label' => esc_html__( 'Popup', 'unicamp' ),
		] );

		$repeater->add_control( 'overlay_display', [
			'label'   => esc_html__( 'Display', 'unicamp' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__( 'Hover', 'unicamp' ),
				'always' => esc_html__( 'Always', 'unicamp' ),
			],
			'default' => '',
		] );
		...
		$repeater->add_control( 'image', [
			...
			'description' => esc_html__( 'Choose an image that displays on left info box', 'unicamp' ),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => 'unicamp-control-media-auto',
		] );
		...
		$this->add_group_control( Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__( 'Popup Image Size', 'unicamp' ),
	...
	private function add_map_markers_style_section() {
		$this->start_controls_section( 'markers_popup_style_section', [
			'label' => esc_html__( 'Markers Popup', 'unicamp' ),
		...
		$this->add_responsive_control( 'markers_popup_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .unicamp-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'markers_popup_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .unicamp-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function get_overlay_value( $settings, $marker, $position ) {
		...
		$this->add_render_attribute( $item_key, 'class', [
			'unicamp-map-overlay',
			"unicamp-map-overlay-{$marker['overlay_display']}",
		...
		if ( $marker['title'] !== '' || $marker['content'] !== '' ) {
			$info_template .= '<div class="unicamp-map-overlay-template"><div class="unicamp-map-overlay-content"><div class="unicamp-map-overlay-info">';

			if ( isset( $marker['image'] ) ) {
				$size  = \Unicamp_Image::elementor_parse_image_size( $settings, '80x9999', 'popup_image_size' );
	...
	private function get_map_style() {
		return [
			'no_label_bright_colors'  => esc_html__( 'No Label Bright Colors', 'unicamp' ),
			'grayscale'               => esc_html__( 'Grayscale', 'unicamp' ),
			'subtle_grayscale'        => esc_html__( 'Subtle Grayscale', 'unicamp' ),
			'apple_paps_esque'        => esc_html__( 'Apple Maps-esque', 'unicamp' ),
			'pale_dawn'               => esc_html__( 'Pale Dawn', 'unicamp' ),
			'midnight_commander'      => esc_html__( 'Midnight Commander', 'unicamp' ),
			'blue_water'              => esc_html__( 'Blue Water', 'unicamp' ),
			'retro'                   => esc_html__( 'Retro', 'unicamp' ),
			'paper'                   => esc_html__( 'Paper', 'unicamp' ),
			'ultra_light_with_labels' => esc_html__( 'Ultra Light with Labels', 'unicamp' ),
			'shades_of_grey'          => esc_html__( 'Shades Of Grey', 'unicamp' ),
			'easy_light'              => esc_html__( 'Easy Light', 'unicamp' ),
			'wy'                      => esc_html__( 'WY', 'unicamp' ),
		];
	}
	...
	private function get_default_marker_icon() {
		$icon_url = UNICAMP_ELEMENTOR_ASSETS . '/images/map-marker.png';
