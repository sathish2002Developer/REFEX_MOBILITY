...
class Widget_Twitter extends Base {
	...
	public function get_title() {
		return esc_html__( 'Twitter', 'unicamp' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control( 'consumer_key', [
			'label' => esc_html__( 'Consumer Key', 'unicamp' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'consumer_secret', [
			'label' => esc_html__( 'Consumer Secret', 'unicamp' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token', [
			'label' => esc_html__( 'Access Token', 'unicamp' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token_secret', [
			'label' => esc_html__( 'Access Token Secret', 'unicamp' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'username', [
			'label' => esc_html__( 'Twitter Username', 'unicamp' ),
			'type'  => Controls_Manager::TEXT,
		] );
		...
		$this->add_control( 'show_date', [
				...
				'label_on'     => esc_html__( 'Yes', 'unicamp' ),
				'label_off'    => esc_html__( 'No', 'unicamp' ),
				'return_value' => '1',
			]
		);
